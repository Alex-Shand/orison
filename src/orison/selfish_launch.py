...
_NVIDIA_UNLOAD_SEQUENCE = ['nvidia_drm', 'nvidia_uvm', 'nvidia_modeset', 'nvidia']
...
def launch(name: str) -> None:
    ...
    sh.run('systemctl', 'set-default', 'graphical.service', capture=False, audit=True)

    # Manually unload nvidia GPU drivers. This isn't done via blacklisting so
    # the next boot will automatically re-load them
    for driver in _NVIDIA_UNLOAD_SEQUENCE:
        sh.run('rmmod', driver, capture=False, audit=True)
    ...
    for addr in system_manifest.pci_addresses:
        sh.virsh('nodedev-detach', f'pci_{addr.domain}_{addr.bus}_{addr.slot}_{addr.function}', capture=False, audit=True)
